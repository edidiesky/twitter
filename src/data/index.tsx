export const feedData = [
  {
    tweet_id: 1,
    user_id: 1,
    profile_name: "The champion",
    username: "tdarke0",
    tweet_text: 'The National Drug Law Enforcement Agency has arrested a teenage student, Benjamin Nnamani Daberechi, 19, with Methamphetamine drugs concealed inside crayfish.',
      hashtags: "#tbt",
    timestamp: "12/22/2022",
    retweet_count: 246,
    favorite_count: 749,
    is_verified: true,
    tweet_image: ['https://pbs.twimg.com/media/F1LQCgcWAAAmiIh?format=jpg&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Miskolc",
  },
  {
    tweet_id: 2,
    user_id: 2,
    profile_name: "The champion",
    username: "imougenel1",
    tweet_text:"Hello world",
    timestamp: "04/09/2022",
    retweet_count: 112,
    favorite_count: 541,
    is_verified: true,
    tweet_image:[],
    // tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small', 'https://pbs.twimg.com/media/F1Kf1SBX0AAapam?format=jpg&name=900x900', 'https://pbs.twimg.com/media/F1Kf1SBX0AAapam?format=jpg&name=900x900', 'https://pbs.twimg.com/media/F1Kf1SBX0AAapam?format=jpg&name=900x900'],
    image: "./images/boris-khentov.jpg",
    location: "Lille",
  },
  {
    tweet_id: 3,
    user_id: 3,
    profile_name: "The champion",
    username: "dgors2",
    tweet_text: "Duis ac nibh.",
    hashtags: "#fashion",
    timestamp: "07/16/2022",
    retweet_count: 697,
    favorite_count: 390,
    is_verified: false,
    tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Duanjia",
  },
  {
    tweet_id: 4,
    user_id: 4,
    profile_name: "The champion",
    username: "tcattermull3",
    tweet_text:
      "Phasellus sit amet erat. Nulla tempus. Vivamus in felis eu sapien cursus vestibulum. Proin eu mi. Nulla ac enim. In tempor, turpis nec euismod scelerisque, quam turpis adipiscing lorem, vitae mattis nibh ligula nec sem. Duis aliquam convallis nunc. Proin at turpis a pede posuere nonummy. Integer non velit. Donec diam neque, vestibulum eget, vulputate ut, ultrices vel, augue.",
    hashtags: "#beautiful",
    timestamp: "05/05/2022",
    retweet_count: 934,
    favorite_count: 746,
    is_verified: false,
    tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Maddela",
  },
  {
    tweet_id: 5,
    user_id: 5,
    profile_name: "The champion",
    username: "aalvarado4",
    tweet_text:
      "In tempor, turpis nec euismod scelerisque, quam turpis adipiscing lorem, vitae mattis nibh ligula nec sem. Duis aliquam convallis nunc. Proin at turpis a pede posuere nonummy. Integer non velit. Donec diam neque, vestibulum eget, vulputate ut, ultrices vel, augue. Vestibulum ante ipsum primis in faucibus orci luctus et ultrices posuere cubilia Curae; Donec pharetra, magna vestibulum aliquet ultrices, erat tortor sollicitudin mi, sit amet lobortis sapien sapien non mi. Integer ac neque. Duis bibendum.",
    hashtags: "#cute",
    timestamp: "03/22/2022",
    retweet_count: 464,
    favorite_count: 562,
    is_verified: true,
    tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Rejoagung",
  },
  {
    tweet_id: 6,
    user_id: 6,
    profile_name: "The champion",
    username: "amadgewick5",
    tweet_text: 'The National Drug Law Enforcement Agency has arrested a teenage student, Benjamin Nnamani Daberechi, 19, with Methamphetamine drugs concealed inside crayfish.',
    hashtags: "#cute",
    timestamp: "10/24/2022",
    retweet_count: 818,
    favorite_count: 56,
    is_verified: false,
    tweet_image: ['https://pbs.twimg.com/media/F1Kw6vnXgAAFsyE?format=jpg&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "El Real",
  },
  {
    tweet_id: 7,
    user_id: 7,
    profile_name: "The champion",
    username: "mlyptratt6",
    tweet_text:
      "Integer pede justo, lacinia eget, tincidunt eget, tempus vel, pede. Morbi porttitor lorem id ligula. Suspendisse ornare consequat lectus.",
    hashtags: "#cute",
    timestamp: "03/22/2022",
    retweet_count: 344,
    favorite_count: 442,
    is_verified: false,
    tweet_image: ['https://pbs.twimg.com/media/F1Kw6vnXgAAFsyE?format=jpg&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Krasnopillya",
  },
  {
    tweet_id: 8,
    user_id: 8,
    profile_name: "The champion",
    username: "gsterte7",
    tweet_text:
      "Etiam faucibus cursus urna. Ut tellus. Nulla ut erat id mauris vulputate elementum. Nullam varius. Nulla facilisi.",
    hashtags: "#love",
    timestamp: "04/27/2022",
    retweet_count: 80,
    favorite_count: 965,
    is_verified: false,
    tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Kurjan",
  },
 
  {
    tweet_id: 12,
    user_id: 12,
    profile_name: "The champion",
    username: "kcreesb",
    tweet_text:
      "Morbi vestibulum, velit id pretium iaculis, diam erat fermentum justo, nec condimentum neque sapien placerat ante. Nulla justo. Aliquam quis turpis eget elit sodales scelerisque. Mauris sit amet eros. Suspendisse accumsan tortor quis turpis. Sed ante. Vivamus tortor. Duis mattis egestas metus. Aenean fermentum. Donec ut mauris eget massa tempor convallis.",
    hashtags: "#love",
    timestamp: "06/07/2022",
    retweet_count: 475,
    favorite_count: 439,
    is_verified: false,
    tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Santana de Parnaíba",
  },
  {
    tweet_id: 13,
    user_id: 13,
    profile_name: "The champion",
    username: "dstilldalec",
    tweet_text:
      "Morbi vestibulum, velit id pretium iaculis, diam erat fermentum justo, nec condimentum neque sapien placerat ante. Nulla justo. Aliquam quis turpis eget elit sodales scelerisque. Mauris sit amet eros. Suspendisse accumsan tortor quis turpis. Sed ante.",
    hashtags: "#fashion",
    timestamp: "01/19/2022",
    retweet_count: 4,
    favorite_count: 575,
    is_verified: false,
    tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Kanaya",
  },
  {
    tweet_id: 14,
    user_id: 14,
    profile_name: "The champion",
    username: "mlowndesbroughd",
    tweet_text: "In congue.",
    hashtags: "#beautiful",
    timestamp: "01/16/2022",
    retweet_count: 309,
    favorite_count: 240,
    is_verified: true,
    tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Tebluru",
  },
  {
    tweet_id: 15,
    user_id: 15,
    profile_name: "The champion",
    username: "mhasnneye",
    tweet_text:
      "Nulla tellus. In sagittis dui vel nisl. Duis ac nibh. Fusce lacus purus, aliquet at, feugiat non, pretium quis, lectus. Suspendisse potenti. In eleifend quam a odio. In hac habitasse platea dictumst. Maecenas ut massa quis augue luctus tincidunt. Nulla mollis molestie lorem. Quisque ut erat.",
    hashtags: "#cute",
    timestamp: "07/25/2022",
    retweet_count: 292,
    favorite_count: 299,
    is_verified: false,
    tweet_image: ['https://pbs.twimg.com/media/F001-85aIAAgXuI?format=png&name=small'],
    image: "./images/boris-khentov.jpg",
    location: "Agodim",
  },
];
